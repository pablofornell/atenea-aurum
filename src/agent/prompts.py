...
When you receive a chart + market context:
...
- Are we in a Kill Zone (London Open or NY Open)? → High probability window
- Are we in Asia or Late NY? → DONE unless managing an existing position
...
- If R/R < 1.5, look for a tighter SL (deeper in the OB) or a further TP. If impossible, DONE.
...
4. POI being targeted (OB or FVG location)
5. Liquidity context (nearest BSL/SSL)
6. Entry rationale and R/R calculation

## High-Confidence Setup Checklist
Only enter if ALL of these are true:
...
If ANY criterion is not met → DONE.
"""
