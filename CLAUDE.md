...
Supports two modes: **demo** (XAUUSD) and **prod** (XAUUSD.mm). Both connect to the EA on port 5555. Default is always `demo`.
...
| `src/config.py` | Global constants. Mode-aware: reads `AURUM_MODE` env var and sets `SYMBOL`, `MT4_PORT`, `STATE_FILE` from the active profile. |
...
| `src/bridge/AURUM_Bridge.mq4` | Expert Advisor in MT4 that serves the socket on port 5555. |
...
| `state/structural_state_demo.json` | Persistent structural state for demo mode (auto-created). |
| `state/structural_state_prod.json` | Persistent structural state for prod mode (auto-created). |
| `state/structural_state_<mode>.previous.json` | Backup of the previous cycle's state (auto-created). |
...
```bash
python aurum.py                        # start in demo mode (XAUUSD, port 5555)
python aurum.py --mode prod            # start in prod mode  (XAUUSD.mm, port 5556)
python aurum.py --reset-bot-state      # reset bot interpretive memory in demo
python aurum.py --mode prod --reset-bot-state  # same, for prod
python tests/tui_demo.py               # preview the TUI with simulated data
python tests/test_mt4_connection.py            # test MT4 connection (demo)
python tests/test_mt4_connection.py --mode prod  # test MT4 connection (prod)
```

## MT4 setup for two modes
...
Attach the EA to the chart matching the mode you intend to run. Only one bot process and one chart should have the EA active at a time.
...
- Single symbol per mode (XAUUSD in demo, XAUUSD.mm in prod). No simultaneous multi-position by design.
- Structured logs in `logs/aurum_session_*.log` and `logs/aurum_decisions_*.jsonl` (manual rotation).
- Structural state persists across cycles in `state/structural_state_<mode>.json`. Two sections:
